...
class NewClientController extends \Core\Controller
{
    ...
    public function __invoke($request, $response, $args)
    {
        ...
        if(false === $request->getAttribute('csrf_status')){
            $this->logger->info('['.$ip.'] NEWCLIENT_CSRF_REJECTED -> EXIT_WITH_403');
            return $response->write($this->getSecurityAlert())->withStatus(403);
        }elseif(empty($name) || empty($email) || empty($pwd)){
            $this->logger->info('['.$ip.'] NEWCLIENT_EMPTY_FIELDS_REJECTION');
    ...
    private function validateClient($email)
    {
        try{
            ...
            $this->errors[] = 'Vous êtes déjà un utilisateur enregistrer.';
            return false;
    ...
    private function sendClientMail($datas)
    {
        ...
        $_subject = 'Inscription au service MangoPay-Payments d\'Ipefix';
    ...
    {
        $alert = '<h4 class="result mid-red">'.(!empty($alert) ? $alert:'Alerte de sécurité').'  <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span></h4>';
